...
const MODULES: Module[] = [
  { name: "Employee Management", status: "live", description: "Employee CRUD, TIN/PFA/bank account (NUBAN) capture, self-service account linking via invite, department assignment for cost-centre reporting, date of birth / nationality capture, and directory search/filter by name, status and department. DOB isn't yet wired into any pension-age eligibility rule and nationality isn't yet wired into expatriate PAYE treatment — both are open statutory questions this build doesn't guess at. Branches/locations and job grades are not built." },
  ...
  { name: "Payroll Processing", status: "partial", description: "Atomic pay-run creation with a balanced double-entry ledger; new-hire pay is prorated to actual days employed this period. Admin-only reversal posts a correcting journal entry for a finalised run — it does not restore loan/expense/leave/attendance/overtime side effects or address already-remitted statutory amounts, both flagged as open questions. Termination and mid-period salary-change proration are not built." },

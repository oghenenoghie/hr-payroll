...
const MODULES: Module[] = [
  ...
  { name: "Payroll Processing", status: "partial", description: "Atomic pay-run creation with a balanced double-entry ledger; new-hire pay is prorated to actual days employed this period, and termination mid-period proration is handled by Final Settlement. Each regular-frequency (weekly/biweekly/monthly) run's detail page flags any employee whose gross pay moved 25%+ from their most recent prior run of the same frequency — informational only, since there's no draft/approval gate in the pay-run state model to block on yet; reversal is still how a flagged run gets corrected. Admin-only reversal posts a correcting journal entry for a finalised run — it does not restore loan/expense/leave/attendance/overtime side effects or address already-remitted statutory amounts, both flagged as open questions. Mid-period salary-change proration is not built (it needs an effective-dated compensation history model this build doesn't have yet), and there's no draft → preview → approved → locked state model for a run — creation is immediate and atomic." },

...
const MODULES: Module[] = [
  ...
  { name: "Payroll Processing", status: "partial", description: "Regular (weekly/biweekly/monthly), 13th Month, Bonus and Arrears runs now land as a Draft — every side effect (loan repayments, expense/overtime/leave-encashment payouts, unpaid-leave and absence deductions) posts immediately so the draft's numbers are real, but the run itself isn't final: an admin/payroll_manager must explicitly Approve it before it counts toward reports, reconciliation, tax certificates or GL export, or Discard it instead, which fully reverses every one of those side effects (loans/expenses/leave/attendance/overtime restored, journal entries deleted, the run itself removed) rather than just correcting the ledger the way reversal does. The 25%+ gross-variance flag on a regular-frequency run's detail page now has real teeth — it's the signal to discard before approving, not just to reverse after the fact. Final Settlement is deliberately not part of this gate: it still posts immediately, since its pay run is a required, non-discardable marker of settlement eligibility rather than a run someone might want to undo. Admin-only reversal remains the correction path for an already-approved run — it posts a correcting journal entry but does not restore side effects or address already-remitted statutory amounts, both still flagged as open questions. New-hire pay is prorated to actual days employed this period, termination mid-period proration is handled by Final Settlement, and a mid-period basic/housing/transport change splits that period's pay across the old and new rate by calendar days, using an auto-logged compensation-history table (mirrors the employee status-history pattern — never a client-chosen effective date, just the day the change was saved). Only the earliest change within a period is treated as the split boundary; a rare second change in the same period isn't given its own segment, a disclosed simplification." },

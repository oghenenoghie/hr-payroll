...
const MODULES: Module[] = [
  ...
  { name: "Payroll Processing", status: "partial", description: "Regular (weekly/biweekly/monthly), 13th Month, Bonus and Arrears runs now land as a Draft — every side effect (loan repayments, expense/overtime/leave-encashment payouts, unpaid-leave and absence deductions) posts immediately so the draft's numbers are real, but the run itself isn't final: an admin/payroll_manager must explicitly Approve it before it counts toward reports, reconciliation, tax certificates or GL export, or Discard it instead, which fully reverses every one of those side effects (loans/expenses/leave/attendance/overtime restored, journal entries deleted, the run itself removed) rather than just correcting the ledger the way reversal does. A regular-frequency run is also checked for variance — an employee's gross moving 30%+ from their last payslip, or someone active and paid last time missing from this run — and unlike the earlier version of this check, it's now a real block: Approve refuses to post a draft with unreviewed flags until you either fix what caused them or explicitly approve again to acknowledge and proceed. Final Settlement is deliberately not part of this gate: it still posts immediately, since its pay run is a required, non-discardable marker of settlement eligibility rather than a run someone might want to undo. Admin-only reversal remains the correction path for an already-approved run — it posts a correcting journal entry and now also restores every side effect the run consumed (loan balances, expense/leave/overtime/leave-encashment approvals back to a re-payable state), the same restoration discard already gives a draft. What it still doesn't do: address already-remitted statutory amounts, or say whether a reversal after a filing deadline requires an amended filing — both remain open questions. New-hire pay is prorated to actual days employed this period, termination mid-period proration is handled by Final Settlement, and a mid-period basic/housing/transport change splits that period's pay across the old and new rate by calendar days, using an auto-logged compensation-history table (mirrors the employee status-history pattern — never a client-chosen effective date, just the day the change was saved). Only the earliest change within a period is treated as the split boundary; a rare second change in the same period isn't given its own segment, a disclosed simplification." },

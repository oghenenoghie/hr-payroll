...
const MODULES: Module[] = [
  ...
  { name: "Payroll Processing", status: "partial", description: "Atomic pay-run creation with a balanced double-entry ledger; new-hire pay is prorated to actual days employed this period, termination mid-period proration is handled by Final Settlement, and a mid-period basic/housing/transport change now splits that period's pay across the old and new rate by calendar days, using an auto-logged compensation-history table (mirrors the employee status-history pattern — never a client-chosen effective date, just the day the change was saved). Only the earliest change within a period is treated as the split boundary; a rare second change in the same period isn't given its own segment, a disclosed simplification. Each regular-frequency (weekly/biweekly/monthly) run's detail page flags any employee whose gross pay moved 25%+ from their most recent prior run of the same frequency — informational only, since there's no draft/approval gate in the pay-run state model to block on yet; reversal is still how a flagged run gets corrected. Admin-only reversal posts a correcting journal entry for a finalised run — it does not restore loan/expense/leave/attendance/overtime side effects or address already-remitted statutory amounts, both flagged as open questions. There's still no draft → preview → approved → locked state model for a run — creation is immediate and atomic." },

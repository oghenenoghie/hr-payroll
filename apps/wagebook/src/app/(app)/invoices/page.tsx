...
export default async function InvoicesPage({
  ...
  const [{ data: queue }, { data: settled, count }, { data: customers }, { data: org }] = await Promise.all([
    supabase
  ...
  // Outstanding balance per issued invoice — derived on read via the
  // shared helper (see the partial-payments migration). Depends on
  // `issued` above, so it's a genuinely sequential second round-trip
  // rather than folded into the first Promise.all.
  const issuedInvoiceIds = issued.map((i) => i.id);
  const settledByInvoice = await getSettledByInvoice(supabase, issuedInvoiceIds);
  function outstandingKobo(invoice: { id: string; amount_kobo: number }): bigint {
    return invoiceOutstandingKobo(invoice, settledByInvoice);
  }
  ...
  return (
    <div className="mx-auto flex w-full max-w-[960px] flex-col gap-5 px-6 py-10">
              ...
              <tbody>
                {drafts.map((invoice) => (
                  <tr key={invoice.id} className="border-b border-border last:border-b-0">
                    <td className={`${tdClass} font-bold text-ink`}>{invoice.invoice_number}</td>
      ...
      {canManage && (
        <div id="raise-invoice" className="scroll-mt-4 rounded-card border border-border bg-surface p-6">
          <span className="text-[11px] font-bold uppercase tracking-[0.03em] text-ink-soft">Raise an invoice</span>
          <div className="mt-3">
            <InvoiceForm customers={customers ?? []} vatRateScaled={vatRateScaled} defaultCustomerId={defaultCustomerId} />
          </div>
        </div>
      )}
    </div>

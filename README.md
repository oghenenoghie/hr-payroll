...
Beyond payroll, Plutus covers the full employer surface: employees, departments, branches, job grades, org chart, recruitment, leave & attendance, loans & advances, expenses, benefits, performance, learning, employment-contract e-signature, and a real accounting layer — vendors/bills (Accounts Payable, with recurring bills and batch payment), customers/invoices (Accounts Receivable, with VAT/line items), chart of accounts, general ledger, financial statements, bank reconciliation, fixed assets, and budgets — with role-scoped RLS, an audit log, and salary masking.

## Product architecture — three pillars

| Pillar | What it does |
|---|---|
| **Payroll Core** | Multi-frequency runs, itemised digital payslips, arrears, bonuses, 13th-month handling, full audit trails, and cumulative recalculation whenever pay changes mid-year. Reversal is gated on acknowledged statutory remittance so a run can't be undone once money has left the building. |
...
| **People Operations** | Leave, attendance, loans & advances, expenses, benefits, final settlement, employment-contract e-signature, and employee/manager self-service — one system of record. |
...
│           │   ├── vendors/    │   ├── customers/     ├── budgets/
│           │   ├── fixed-assets/│  ├── team/          ├── notifications/
│           │   └── me/  (self-service)
...
- **Audit-ready by default** — every cycle keeps a full trail; liabilities and filing status reportable by entity and state; statutory remittances are recorded per scheme and must be acknowledged before a pay run can be reversed.

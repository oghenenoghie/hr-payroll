-- Attendance (People Operations pillar, distinct from Leave & Attendance's
-- leave-request/balance workflow). "Weekly attendance feeding straight
-- into payroll deductions" — admin/HR mark a day Late or Absent from a
-- weekly grid; Present is the implicit default and is never stored, so
-- marking a whole org for a week doesn't require seeding a row per
-- employee per day. Only Absent reduces pay (via the same daily-rate
-- formula as unpaid leave); Late is tracked for the record but isn't
-- itself a documented statutory or company-policy deduction, so it's
-- deliberately left as a record-only status, not a payroll input.
...
create index attendance_records_org_id_idx on public.attendance_records (org_id);
...
create policy "admins and HR managers can view all attendance records"
on public.attendance_records for select
to authenticated
using (core.has_org_role(org_id, array['admin', 'hr_manager']));

create policy "admins and HR managers can mark attendance"
on public.attendance_records for insert
to authenticated
with check (marked_by = auth.uid() and core.has_org_role(org_id, array['admin', 'hr_manager']));

create policy "admins and HR managers can update attendance records"
on public.attendance_records for update
to authenticated
using (core.has_org_role(org_id, array['admin', 'hr_manager']))
with check (core.has_org_role(org_id, array['admin', 'hr_manager']));

create policy "admins and HR managers can clear attendance records"
on public.attendance_records for delete
to authenticated
using (core.has_org_role(org_id, array['admin', 'hr_manager']));

-- Same reasoning as the reimbursement/leave/benefits columns: store the
-- attendance-absence deduction explicitly rather than folding it silently
-- into the reduced gross figure, so the "· how?" derivation can show it
-- separately from an approved-unpaid-leave deduction (the two are
-- distinct sources — one an employee-approved absence, the other an
-- after-the-fact admin/HR record — even though both use the same
-- daily-rate formula and reduce gross the same way).
alter table public.payslips
  add column attendance_absence_deduction_kobo bigint not null default 0;

-- Converted from security invoker to security definer (with an explicit
-- authorization check that mirrors the pay_runs insert policy it replaces:
-- admin or payroll_manager for this org, nothing wider). This was
-- discovered necessary, not stylistic: a payroll_manager (who is neither
-- admin nor hr_manager) has no direct-table UPDATE rights on
-- leave_requests or attendance_records under their existing admin/
-- hr_manager-only RLS policies, so as security invoker those two internal
-- UPDATEs silently affected 0 rows for a payroll_manager caller — matched
-- by the pre-existing "not found" exceptions, so the failure looked like
-- a business-rule rejection rather than the real authorization gap. Same
-- root cause and fix shape as review_leave_request's security-definer
-- conversion in the Manager Self-Service phase.
...
revoke all on function public.create_pay_run(jsonb) from public, anon;

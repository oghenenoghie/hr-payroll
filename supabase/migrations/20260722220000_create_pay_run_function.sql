-- Atomic pay-run writer. All statutory math happens in TypeScript
-- (packages/compliance) — this function is a thin, transactional
-- persistence layer only, so the rules-as-data principle isn't
-- duplicated into SQL. security invoker (the default) means every
-- insert below still goes through the normal RLS policies for the
-- calling user — this function grants no privilege the caller didn't
-- already have via their org role.
--
-- Wrapping pay_runs + journal_entries + payslips + ledger_postings in
-- one plpgsql function makes the whole run atomic: any failure (a
-- rejected RLS check, a bad payload, an unbalanced journal entry caught
-- by core.validate_journal_balance()) rolls back everything, so a
-- pay run can never partially post.

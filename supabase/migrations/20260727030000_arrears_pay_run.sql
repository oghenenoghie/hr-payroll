-- Arrears/retroactive pay: an admin-entered, per-employee lump sum for a
-- shortfall from a prior period being paid out now (a late-processed
-- raise, a corrected payroll error) — same shape as bonus, reusing
-- deriveLumpSumPayslip(kind: "arrears").
--
-- feature-backlog.md §1 flagged a genuinely open statutory question here:
-- arrears paid under one rule regime for work performed under a prior
-- one — which rule version applies, period earned or period of receipt?
-- Resolved (product decision, not yet confirmed against a Nigerian tax
-- professional) as period of receipt: arrears is taxed under whichever
-- RuleVersion is in force when the pay run posts, same as every other
-- lump sum, via the run's existing rule_version_id — no per-earning-
-- period RuleVersion pinning was added, since that's the far larger,
-- data-model-level change the backlog entry described as "expensive to
-- reverse" and out of scope for this decision.
--
-- arrears_note is required at the application layer (never here) and
-- records which period the arrears relates to and why — the audit
-- context feature-backlog.md's flagged gap cared about, kept as
-- free text rather than a second tax calculation against a historical
-- rule version.
alter table public.pay_runs
  drop constraint pay_runs_frequency_check,
  add constraint pay_runs_frequency_check
    check (frequency in ('weekly', 'biweekly', 'monthly', 'off-cycle', 'thirteenth_month', 'bonus', 'arrears'));

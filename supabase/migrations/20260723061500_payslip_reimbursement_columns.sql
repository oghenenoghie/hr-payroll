-- Store the reimbursement split explicitly on the payslip rather than
-- leaving it implicit in gross_kobo — the "· how?" derivation is the
-- product's core demonstration, and it can't show what it can't see.
alter table public.payslips
  add column taxable_reimbursement_kobo bigint not null default 0,
  add column non_taxable_reimbursement_kobo bigint not null default 0;

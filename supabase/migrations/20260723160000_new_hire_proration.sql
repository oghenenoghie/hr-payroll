-- New-hire proration — the top-priority correctness gap flagged in the
-- plutus-payroll-platform skill's feature-backlog.md §1: without it, an
-- employee hired mid-period still receives a full period's base pay from
-- day one. Termination proration is already handled by Final Settlement
-- (terminated employees are excluded from regular runs entirely); mid-
-- period salary-change proration is a separate, larger data-model change
-- (effective-dated compensation history) and stays an open gap.
--
-- Proration basis: calendar days, matching the existing daily-rate
-- formula already used for unpaid leave and attendance-absence deductions
-- (annual contractual pay ÷ 365) — a disclosed simplification, not a
-- Labour Act figure, so no new RuleVersion field is needed here. Stored
-- as its own payslip column (not folded into the existing deduction
-- columns) so the "· how?" derivation can show it as a distinct, audited
-- line rather than silently reducing gross.
alter table public.payslips
  add column new_hire_proration_deduction_kobo bigint not null default 0;
...
begin
  ...
  loop
    insert into public.payslips (
      ...
      overtime_pay_kobo, new_hire_proration_deduction_kobo
    )
    values (
      v_pay_run.id,

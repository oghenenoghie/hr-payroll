...
- ~~**Payroll variance and anomaly detection**~~ — flag month-over-month anomalies before a run is approved. A natural fit for the "correct by construction" thesis: catching a fat-fingered salary before it's paid is worth more than reporting it after. **Built, with a real scope caveat:** the pay run detail page flags any employee whose gross pay moved ≥25% from their most recent prior payslip of the *same* regular frequency (weekly/biweekly/monthly — bonus, 13th month and final settlement are inherently one-off and excluded from the comparison entirely). This is computed on page load, same as every other report in this app, and is informational only — it can't block "approval" of a run because there is no draft/preview/approval gate in the pay-run state model yet (see "Payroll locking and finalisation" above). Reversal remains the mechanism to correct a flagged run that turns out to be wrong.

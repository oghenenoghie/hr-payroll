...
Enrollment currently means one form. Real onboarding is a checklist with states: documentation, contract signing, probation tracking, confirmation — none of this is built. Offboarding likewise: ~~notice period~~, ~~exit checklist~~, ~~asset return~~, ~~**access revocation**~~, ~~clearance~~, final settlement, ~~experience letter~~. **Built:** access revocation (the app layout gate reads the linked employee's `status` on every request — not a cached session claim — and redirects to `/account-revoked` the instant it's `terminated`, regardless of `org_membership` role; the page re-checks live on load too, so a reinstated employee isn't stuck behind a stale gate) plus an `employee_offboarding_checklist` covering the remaining manual steps — notice period served, assets returned, clearance obtained, experience letter issued — as one admin/HR-editable row per terminated employee on the edit page, shown alongside read-only indicators for the two steps that were already automatic or tracked elsewhere (access revoked, final settlement processed). The checklist only tracks whether an experience letter was issued — it does not generate, template or e-sign one. Onboarding's own checklist (documentation, contract signing, probation tracking, confirmation) remains unbuilt.

...
Employment contracts and contract-expiry alerting · employee documents repository · ~~probation tracking and confirmation~~ **(built: probation_end_date + a confirmed flag, surfaced as an Overdue/Ends soon/On probation badge in the employee directory; deliberately not wired into payroll math — Nigerian law doesn't condition PAYE/pension/NHF on probation status)** · organisation chart and reporting hierarchy · job grades and salary bands · employee directory search and filtering · company policies with acknowledgement.
